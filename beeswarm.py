...
#         cb.ax.set_aspect((bbox.height - 0.9) * 20)
    ...
    pl.xlabel(labels['VALUE'], fontsize=14)
    if show:
        pl.show()
...
def summary_legacy(shap_values, features=None, feature_names=None, max_display=None, plot_type=None,
    ...
    if not multi_class and len(shap_values.shape) == 3:
        ...
        pl.figure(figsize=(8.27*0.65, 8.27*0.65))
        pl.subplot(1, max_display, 1)
...
#         cb.ax.set_aspect((bbox.height - 0.9) * 20)
    ...
    pl.gca().spines['left'].set_visible(False)
    pl.gca().tick_params(color=axis_color, labelcolor=axis_color)
    pl.yticks(range(len(feature_order)), [feature_names[i] for i in feature_order], fontsize=10)
    if plot_type != "bar":
        pl.gca().tick_params('y', length=20, width=0.5, which='major')
    pl.gca().tick_params('x', labelsize=14)
    pl.ylim(-1, len(feature_order))
    if plot_type == "bar":
        pl.xlabel(labels['GLOBAL_VALUE'], fontsize=14)
    else:
        pl.xlabel(labels['VALUE'], fontsize=14)
    pl.tight_layout()
    if show:
